
# Ordini Professionali

## Ordine della Galassia di Andromeda
- **Descrizione**: Gli chef iscritti e operanti nel suddetto Ordine sono tenuti a considerare e gestire una peculiare caratteristica fisiologica della propria utenza: tutti gli abitanti di Andromeda manifestano intolleranza al lattosio.

## Ordine dei Naturalisti
- **Descrizione**: L'Ordine venera la purezza originaria del cibo. Il loro credo si basa sul 'Principio di Non Alterazione': un ingrediente può essere modificato solo fino al punto in cui mantiene ancora riconoscibile la sua essenza originaria.

## Ordine degli Armonisti
- **Descrizione**: Gli chef sono dotati di una rara sensibilità che permette loro di preparare piatti in grado di entrare in sintonia con le frequenze emotive dei loro ospiti. La filosofia si basa sul principio che il cibo deve entrare in risonanza con lo stato emotivo di chi lo consuma.

---

# Sostanze Regolamentate

## Sostanze Psicotrope e Psioniche
- **Erba Pipa**
  - CRP: 0.89
  - Limiti: Limite massimo del 5% in massa o volume rispetto alla preparazione totale.
  
- **Cristalli di Memoria**
  - CRP: 0.92
  - DeltaQ: 0.2
  - Limiti: Limite massimo del 5% in massa o volume rispetto alla preparazione totale.
  
- **Petali di Eco**
  - CRP: 0.87
  - IPM: 0.6
  - Limiti: Limite massimo del 5% in massa o volume rispetto alla preparazione totale.

## Ingredienti di Origine Mitica
- **Carne di Drago**
  - IPM: 0.85
  - IBX: 0.75
  - Limiti: Limite massimo del 5% in massa o volume rispetto alla preparazione totale.
  
- **Uova di Fenice**
  - IPM: 0.98
  - IEI: 0.8
  - Limiti: Limite massimo del 5% in massa o volume rispetto alla preparazione totale.
  
- **Lacrime di Unicorno**
  - IPM: 0.95
  - Theta: 0.9
  - Limiti: Limite massimo del 5% in massa o volume rispetto alla preparazione totale.

## Sostanze Xenobiologiche
- **Foglie di Mandragora**
  - IBX: 0.82
  - MI: 0.3
  - Limiti: Limite massimo del 5% in massa o volume rispetto alla preparazione totale.
  
- **MuNa Lunare**
  - CRP: 0.72
  - IBX: 0.78
  - MI: 0.6
  - Limiti: Limite massimo del 5% in massa o volume rispetto alla preparazione totale.
  
- **Nettare di Sirena**
  - IBX: 0.85
  - MI: 0.2
  - Theta: 0.7
  - Limiti: Limite massimo del 5% in massa o volume rispetto alla preparazione totale.

## Sostanze Quantiche e Dimensionali
- **Spore Quantiche**
  - DeltaQ: 0.45
  - CDT: 0.8
  - Limiti: Limite massimo del 5% in massa o volume rispetto alla preparazione totale.
  
- **Essenza di Vuoto**
  - DeltaQ: 0.16
  - CDT: 0.95
  - Limiti: Limite massimo del 5% in massa o volume rispetto alla preparazione totale.
  
- **Funghi dell’Etere**
  - DeltaQ: 0.38
  - CDT: 0.75
  - Limiti: Limite massimo del 5% in massa o volume rispetto alla preparazione totale.

## Sostanze Spazio-Temporali
- **Sale Temporale**
  - CDT: 0.65
  - Limiti: Limite massimo del 5% in massa o volume rispetto alla preparazione totale.
  
- **Radici di Gravità**
  - CDT: 0.55
  - Limiti: Limite massimo del 5% in massa o volume rispetto alla preparazione totale.
  
- **Polvere di Stelle**
  - IPM: 0.88
  - CDT: 0.75
  - Limiti: Limite massimo del 5% in massa o volume rispetto alla preparazione totale.

---

# Tecniche Culinarie

## Tecniche di Preparazione
- **Marinatura**
  - Descrizione: La marinatura a infusione gravitazionale richiede una licenza gravitazionale (G) di livello II.
  - Licenze richieste: Gravitazionale 2
  - Grado tecnologico: LTK 2

- **Affumicatura**
  - Descrizione: L'affumicatura a strati catione quantica richiede una licenza quantistica (Q) di livello 3 e un grado tecnologico LTK di livello II.
  - Licenze richieste: Quantistica 3
  - Grado tecnologico: LTK 2

- **Fermentazione**
  - Descrizione: La fermentazione quantica a strati multiversali richiede una licenza quantistica (Q) di livello 5 e un grado tecnologico LTK di livello VI+.
  - Licenze richieste: Quantistica 5
  - Grado tecnologico: LTK 6

- **Decostruzione**
  - Descrizione: La decostruzione atomica a strati energetici richiede una licenza antimaterica (e+) di livello I, una licenza quantistica (Q) di livello 1 e un grado tecnologico LTK di terzo (II) livello.
  - Licenze richieste: Antimateria 1, Quantistica 1
  - Grado tecnologico: LTK 2

- **Sfericazione**
  - Descrizione: La sfericazione a gravità psionica variabile richiede una licenza psionica (P) di livello IV, una licenza gravitazionale (G) di livello I e un grado tecnologico LTK di terzo (III) livello.
  - Licenze richieste: Psionica 4, Gravitazionale 1
  - Grado tecnologico: LTK 3

---

# Licenze Disponibili

- **Licenza Psionica**
  - Categoria: Psionica
  - Livello: 0

- **Licenza Gravitazionale**
  - Categoria: Gravitazionale
  - Livello: 0

- **Licenza Antimateria**
  - Categoria: Antimateria
  - Livello: 0

- **Licenza Magnetica**
  - Categoria: Magnetica
  - Livello: 0

- **Certificazione di grado tecnologico LTK**
  - Categoria: Tecnologico
  - Livello: 1

---

# Sanzioni

- **Violazione dei limiti quantitativi stabiliti per le sostanze con risonanza psionica**
  - Categorie coinvolte: Sostanze Psicotrope e Psioniche
  - Gravità: Alta
  - Penalità: Obbligo di sottoporsi a una valutazione psionica approfondita.

- **Falsificazione delle certificazioni dell'Ordine dei Naturalisti**
  - Categorie coinvolte: Ordine dei Naturalisti
  - Gravità: Alta
  - Penalità: Obbligo di sottoporsi a un percorso di rieducazione sulla filosofia del minimo intervento senziente.
